...
from packnet_sfm.dgp.utils.camera import Camera, generate_depth_map
...
def stack_sample(sample):
    ...
    for key in sample[0]:
        ...
        if key in ['idx', 'dataset_idx']:
            ...
            elif key in ['filename', 'sensor_name']:
                stacked_sample[key] = []
                # Stack list of torch tensors
...
class DGPDataset:
    ...
    def __getitem__(self, idx):
        ...
        self.sample_dgp = [sample for sample in self.sample_dgp]
        if type(self.sample_dgp[0]) != type([]):
            self.sample_dgp = [self.sample_dgp]
